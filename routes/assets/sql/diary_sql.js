...
var diarySqlFun = {
    ...
    /*插入用户*/
    insertUser: function (user_id, name, roleType, className) {
        var info = {
            flag: false,
            message: ""
        };
    ...
    /*更新用户*/
    updateUser: function (user_id, name, roleType, className) {
        var info = {
            flag: false,
            message: ""
        };
    ...
    /*添加日记*/
    insertDiary: function (id, user_id, content, weather, add_time, pass_flag, className) {
        var info = {
            flag: false,
            message: ""
        };
    ...
    getAdminDiaryCount: function (pass_flag, className) {
        var info = {
            flag: false,
            message: ""
        }
        var promise = new Promise(function (resolve, reject) {
            className = ['[三年级,1班]', '[三年级,2班]', '[三年级,1班],[三年级,2班]'];
            sequelize.query('SELECT COUNT(*) FROM (SELECT * FROM postings_table WHERE postings_table.user_id IN(SELECT user_etzr.user_id FROM user_etzr WHERE user_etzr.class IN(?)) AND postings_table.del_flag = 0 AND postings_table.pass_flag = ? AND postings_table.post_type = 1 ) AS p', {
                replacements: [className, pass_flag],
    ...
    getAdminPostcomCount: function (className) {
        var info = {
            flag: false,
            message: ""
        }
        var promise = new Promise(function (resolve, reject) {
            className = ['[三年级,1班]', '[三年级,2班]', '[三年级,1班],[三年级,2班]'];
            sequelize.query('SELECT COUNT(*) FROM (SELECT * FROM postings_table WHERE postings_table.user_id IN(SELECT user_etzr.user_id FROM user_etzr WHERE user_etzr.class IN(?)) AND postings_table.del_flag = 0 AND postings_table.post_type = 3 ) AS p', {
    ...
    //得到佳作相应年份的数量
    getAdminFineworkCount: function (user_id, time) {
        var info = {
            flag: false,
            message: ""
        };
        var promise = new Promise(function (resolve, reject) {
            sequelize.query('SELECT count(*) FROM postings_table WHERE YEAR (new_create_time) = YEAR (?) AND post_type = 2 AND del_flag = 0 AND user_id = ?', {
                replacements: [time, user_id],
                type: sequelize.QueryTypes.SELECT
            }).then(function (result) {
                info.count = result[0]["count(*)"];
                info.message = "查找数量成功";
    ...
    // 后台获取日记信息
    getAdminDiaryAndImages: function (page, order, orderType, pass_flag, className) {
        var info = {
            flag: false,
            message: ""
        };
        var promise = new Promise(function (resolve, reject) {
            ...
            className = ['[三年级,1班]', '[三年级,2班]', '[三年级,1班],[三年级,2班]'];
            page = parseInt(page - 1) * pn;
            var query = 'SELECT p.*, GROUP_CONCAT(img.resource_url) AS imgData FROM (SELECT * FROM postings_table WHERE postings_table.user_id IN(SELECT user_etzr.user_id FROM user_etzr WHERE user_etzr.class IN(?)) AND postings_table.post_type = 1 AND postings_table.del_flag = 0 AND postings_table.pass_flag = ? LIMIT ?, 10) AS p LEFT JOIN resouce_table AS img ON p.id = img.parent_id GROUP BY p.id ORDER BY ' + orderType + ' ' + order;
            console.log(query);
            sequelize.query(query, {
                replacements: [className, pass_flag, page],
                type: sequelize.QueryTypes.SELECT
            }).then(function (result) {
                if (result.length) {
                    for (var i = 0; i < result.length; i++) {
                        result[i].new_create_time = moment(result[i].new_create_time).format("YYYY-MM-DD HH:mm:ss");
                    }
                    info.flag = true;
                    info.message = "查找日记成功";
                    info.data = result;
    ...
    //获得日记和作者和身份信息
    getAdminDiaryInfo: function (page, order, orderType, pass_flag, className) {
        var info = {
            flag: false,
            message: ""
        };
        var promise = new Promise(function (resolve, reject) {
            ...
            page = parseInt(page - 1) * pn;
            className = ['[三年级,1班]', '[三年级,2班]', '[三年级,1班],[三年级,2班]'];
            var query = 'SELECT p.id,p.user_id,p.content,u.real_name AS real_name,u.role_type AS roleType FROM (SELECT * FROM postings_table WHERE postings_table.user_id IN(SELECT user_etzr.user_id FROM user_etzr WHERE user_etzr.class IN(?)) AND postings_table.del_flag = 0 AND postings_table.pass_flag = ? AND postings_table.post_type = 1 LIMIT ?, 10) AS p LEFT JOIN user_etzr AS u ON p.user_id = u.user_id ORDER BY ' + orderType + ' ' + order;
            sequelize.query(query, {
                replacements: [className, pass_flag, page],
                type: sequelize.QueryTypes.SELECT
            }).then(function (result) {
    ...
    getAdminFineworkInfo: function (user_id, page, time) {
        var info = {
            flag: false,
            message: "",
            data: {}
        ...
        var promise = new Promise(function (resolve, reject) {
            var pn = 9;
            page = parseInt(page - 1) * pn;
            sequelize.query('SELECT p.*,r.resource_url,r.preview_url,r.img_url_L,r.img_url_m,r.img_url_s FROM postings_table p LEFT JOIN resouce_table r ON p.id=r.parent_id WHERE p.post_type=2 AND p.del_flag=0 AND p.user_id = ? AND YEAR (new_create_time) = YEAR (?) ORDER BY p.new_create_time DESC limit ?,9', {
                replacements: [user_id, time, page],
                type: sequelize.QueryTypes.SELECT
            }).then(function (result) {
                if (result.length) {
                    var data = JSON.parse(JSON.stringify(result));
    ...
    // 后台获取帖子信息图片
    getAdminPostcomAndImages: function (page, order, orderType, className) {
        var info = {
            flag: false,
            message: ""
        };
        var promise = new Promise(function (resolve, reject) {
            ...
            className = ['[三年级,1班]', '[三年级,2班]', '[三年级,1班],[三年级,2班]'];
            page = parseInt(page - 1) * pn;
            var query = 'SELECT p.*, GROUP_CONCAT(img.resource_url) AS imgData FROM (SELECT * FROM postings_table WHERE postings_table.user_id IN(SELECT user_etzr.user_id FROM user_etzr WHERE user_etzr.class IN(?)) AND postings_table.post_type = 3 AND postings_table.del_flag = 0 LIMIT ?, 20) AS p LEFT JOIN resouce_table AS img ON p.id = img.parent_id GROUP BY p.id ORDER BY ' + orderType + ' ' + order;
            console.log(query);
            sequelize.query(query, {
                replacements: [className, page],
                type: sequelize.QueryTypes.SELECT
            }).then(function (result) {
                if (result.length) {
                    info.flag = true;
    ...
    getAdminPostcomInfo: function (page, order, orderType, className) {
        var info = {
            flag: false,
            message: ""
        };
        var promise = new Promise(function (resolve, reject) {
            ...
            page = parseInt(page - 1) * pn;
            className = ['[三年级,1班]', '[三年级,2班]', '[三年级,1班],[三年级,2班]'];
            var query = 'SELECT p.id,p.user_id,p.content,u.real_name AS real_name,u.role_type AS roleType FROM (SELECT * FROM postings_table WHERE postings_table.user_id IN(SELECT user_etzr.user_id FROM user_etzr WHERE user_etzr.class IN(?)) AND postings_table.del_flag = 0 AND postings_table.post_type = 3 LIMIT ?, 20) AS p LEFT JOIN user_etzr AS u ON p.user_id = u.user_id ORDER BY ' + orderType + ' ' + order;
            sequelize.query(query, {
                replacements: [className, page],
                type: sequelize.QueryTypes.SELECT
            }).then(function (result) {
