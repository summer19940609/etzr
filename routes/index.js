...
var systemCode = config.systemCode;
    ...
    request.post({
        ...
        if (err) {
            ...
            if (!body.userInfo) {
                ...
                // 根据userId和token查找用户信息 班级和是否毕业
                request.post({
                    url: ums_url + '/user/getUserInfo',
                    form: {
                        "systemCode": systemCode,
                        "userId": user_id,
                        "token": token
                    ...
                    if (err) {
                        ...
                        if (body.code) {
                            ...
                            var className = body.userInfo.className;
                            var userStatus = body.userInfo.userStatus;
                            userData.className = className;
                            info.data.userData = userData;
                            info.data.token = token;
                            if (userStatus === '毕业') {
                                info.message = '已毕业，登录失败';
                                res.send(info);
                            }
                            ...
                            diarySqlFun.findUserOne(user_id).then(function (data0) {
                                co(function* () {
                                    if (data0.data) {
                                        //如果能查询到数据，执行更新
                                        yield diarySqlFun.updateUser(user_id, nikeName, roleType, className).then(function (data1) {
                                            //更新成功
                                        }, function (data1) {
                                            //更新失败
                                        ...
                                        yield diarySqlFun.insertUser(user_id, nikeName, roleType, className).then(function (data2) {
                                            //插入成功
                                        }, function (data2) {
...
// 后台日记列表
router.post('/diary/admin/list', function (req, res, next) {
    var page = req.body.page;
    var order = req.body.order;
    var orderType = req.body.orderType;
    var pass_flag = req.body.pass_flag;
    ...
    redis.getData(user_id).then(function (result) {
        ...
        if (local_token !== redis_token || !redis_token) {
            ...
            diarySqlFun.getAdminDiaryAndImages(page, order, orderType, pass_flag, className).then(function (data) {
                if (data.data) {
                    co(function* () {
                        var obj = data.data;
                        var nameData;
                        yield diarySqlFun.getAdminDiaryInfo(page, order, orderType, pass_flag, className).then(function (data2) {
                            if (data2.data) {
                                nameData = data2.data;
                            } else {
                                info.flag = true;
                        ...
                        //查询到的日记数量
                        yield diarySqlFun.getAdminDiaryCount(pass_flag, className).then(function (data0) {
                            if (data0.count) {
                                var count = data0.count;
                                info.count = count;
                                var pageNum = Math.ceil(count / 10);
    ...
    upload(req, res, function (err) {
        ...
        redis.getData(user_id).then(function (result) {
            ...
            if (local_token !== redis_token || !redis_token) {
                ...
                diarySql
                Fun.insertFile(fileArr).then(function (data) {
                    return diarySqlFun.insertDiary(id, user_id, content, weather, add_time, pass_flag, className).then(function (data) {
                        info.message = data.message;
                        info.flag = true;
                        res.send(info);
                    }, function (data) {
...
router.post('/finework/admin/list', function (req, res, next) {
    var page = req.body.page;
    var year = req.body.year;
    var time = year + '-01-01';
    var user_id = req.body.user_id;
    ...
    redis.getData(user_id).then(function (result) {
        ...
        if (local_token !== redis_token || !redis_token) {
            ...
            diarySqlFun.getAdminFineworkInfo(user_id, page, time).then(function (data0) {
                    if (data0.data) {
                        co(function* () {
                            yield diarySqlFun.getAdminFineworkCount(user_id, time).then(function (data1) {
                                var count = data1.count;
                                var pageNum = Math.ceil(count / 9);
                                info.pageNum = pageNum;
                            }, function (data1) {
...
//帖子列表
router.post('/postcom/admin/list', function (req, res, next) {
    var page = req.body.page;
    var order = req.body.order;
    var orderType = req.body.orderType;
    var className = req.body.className;
    ...
    redis.getData(user_id).then(function (result) {
        ...
        if (local_token !== redis_token || !redis_token) {
            ...
            diarySqlFun.getAdminPostcomAndImages(page, order, orderType, className).then(function (data) {
                    if (data.data) {
                        co(function* () {
                            var obj = data.data;
                            var nameData;
                            yield diarySqlFun.getAdminPostcomInfo(page, order, orderType, className).then(function (data1) {
                                nameData = data1.data;
                            }, function (data1) {
                                info.message = data1.message;
                                res.send(info);
                            });
                            yield diarySqlFun.getAdminPostcomCount("3").then(function (data0) {
                                if (data0.count) {
                                    var count = data0.count;
                                    info.count = count;
                                    var pageNum = Math.ceil(count / 20);
